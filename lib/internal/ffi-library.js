...
const { kSbInvokeSlow, kSbArguments, kSbReturn, kSbSharedBuffer } = require('./ffi-shared-buffer')
const { validateNoNullBytes } = require('./ffi-errors')
const { signatureKey, validateDefinitionsObject, validateFunctionDefinition } = require('./ffi-signature')
...
function createWrappedFunction(lib, name, spec, options = {}) {
  const { exposeSbMetadata = true } = options
  const raw = (...args) => {
    if (lib._closed) {
      throw new Error('Library is closed')
    }
    if (FLOAT_SPECIAL_RESULT_TYPES.has(spec.return)) {
      const hasNonFiniteArg = args.some((arg) => typeof arg === 'number' && !Number.isFinite(arg))
      if (hasNonFiniteArg && spec.arguments.every((type) => type === spec.return)) {
        return invokeFloatSpecialFallback(spec, args)
      }
    }
    return lib._native.invoke(spec.key, spec.pointer, args)
  }
  const wrapped = wrapFunction(raw, spec.arguments, spec.return, { exposeMetadata: exposeSbMetadata })
  if (!exposeSbMetadata) {
    // no-op: metadata was never attached to this wrapper
  }
...
function createCachedFunction(lib, name, signature, spec) {
  const wrapper = createWrappedFunction(lib, name, spec)
  lib._functionCache.set(name, { signature: signatureKey(signature), spec, wrapper })
  return wrapper
}

function assertCompatibleCachedSignature(lib, name, signature) {
  const signatureId = signatureKey(signature)
  const cached = lib._functionCache.get(name)
  if (cached && cached.signature !== signatureId) {
    throw new Error(`Function '${name}' was already requested with a different signature`)
  }
  return cached
}
...
class DynamicLibrary {
  ...
  getFunction(symbol, definition = {}) {
    ...
    const cached = assertCompatibleCachedSignature(this, symbol, signature)
    if (cached) return cached.wrapper
    const spec = this._native.getFunction(symbol, signature)
    const wrapper = createCachedFunction(this, symbol, signature, spec)
  ...
  getFunctions(definitions) {
    ...
    for (const [name, definition] of Object.entries(definitions)) {
      ...
      const signature = validateFunctionDefinition(name, definition)
      const cached = assertCompatibleCachedSignature(this, name, signature)
      const spec = cached?.spec ?? this._native.getFunction(name, signature)
      pending.push({ cached, name, signature, spec })
    }
    for (const { cached, name, signature, spec } of pending) {
      if (!cached) createCachedFunction(this, name, signature, spec)
      const wrapper = createWrappedFunction(this, name, spec, { exposeSbMetadata: false })
      out[name] = wrapper
      this.functions[name] = wrapper
      this.symbols[name] = spec.pointer
      this._symbolCache.set(name, spec.pointer)
    }
    return out
  }
}
