...
function normalizeBigIntValue(value, label, signed = true) {
  let bigint
...
function createNormalizer(kind) {
  ...
  if (kind === 'int64') return (value) => normalizeBigIntValue(value, 'an int64', true)
  if (kind === 'uint64') return (value) => normalizeBigIntValue(value, 'a uint64', false)
  return (value) => value
}
