...
test('ffi reads and writes integer and float values', () => {
  withAllocations(
    common.mustCall((alloc) => {
      ...
      ffi.setInt64(ptr, 48, -9007199254740991);
      ffi.setUint64(ptr, 56, 9007199254740991);
      ...
      assert.strictEqual(ffi.getInt64(ptr, 48), -9007199254740991n);
      assert.strictEqual(ffi.getUint64(ptr, 56), 9007199254740991n);
...
test('ffi validates memory access arguments', () => {
  withAllocations(
    common.mustCall((alloc) => {
      const ptr = alloc(8);
      const maxPointer = process.arch === 'ia32' || process.arch === 'arm' ? 0xfffffffcn : 0xfffffffffffffffcn;
